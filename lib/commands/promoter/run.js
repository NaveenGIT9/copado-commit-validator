...
class PromoterRun extends SfCommand {
    async run() {
        ...
        let stories = [];
        try {
            const result = await conn.query(`SELECT Id, Name, copado__Org_Credential__c, copado__Project__c, copado__Status__c, copado__Has_Apex_Code__c, copado__Environment__r.Name ` +
        ...
        for (const story of stories) {
            ...
            let apexTestRecordExists = null;
            if (story.copado__Has_Apex_Code__c) {
                try {
                    const testRes = await conn.query(`SELECT Id FROM copado__Result__c WHERE copado__User_Story__c = '${story.Id}' ` +
                    ...
                    try {
                        const testRes2 = await conn.query(`SELECT Id FROM copado__Apex_Test_Result__c WHERE copado__User_Story__c = '${story.Id}' LIMIT 1`);
