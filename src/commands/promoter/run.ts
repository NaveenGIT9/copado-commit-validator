...
interface CommitRecord {
  copado__Snapshot_Commit__r: { copado__Commit_Id__c: string } | null;
}

export default class PromoterRun extends SfCommand<void> {
  public static readonly summary = 'Verify and promote Copado User Stories';
  ...
  public async run(): Promise<void> {
    ...
    const storyNames = (flags.stories as string).split(',').map((s) => s.trim()).filter(Boolean);
    const repoPath = (flags['repo-path'] as string | undefined) ?? process.cwd();
    const doPromote = (flags.promote as boolean | undefined) ?? false;
    const conn = flags['target-org'].getConnection() as unknown as Connection;
    ...
    if (doPromote && flags['story-ids']) {
      ...
      for (const id of ids) {
        ...
        try {
          await (conn.sobject('copado__User_Story__c') as unknown as {
            update: (data: Record<string, unknown>) => Promise<unknown>;
          }).update({ Id: id, copado__Promote_and_Deploy__c: true });
    ...
    for (const story of stories) {
      ...
      let copadoCommits: string[] = [];
      try {
        const commitResult = await conn.query<CommitRecord>(
